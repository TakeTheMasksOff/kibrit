<li class='<?php echo ($item->keyword == strtolower(Yii::app()->controller->action->id)?'active ':'');?>'>
  ...
  <ul class="subMenu">
    ...
    foreach( $blogs as $row )
    {
        ...
        $detail = Cleanurls::getUrlOrSave($element,$element->getTranslation($this->Lang)->name?$element->getTranslation($this->Lang)->name:'',$this->Lang);
        $item_n = $row->getTranslation($this->Lang)->name;
    ?>
        <?php if ($element->getContentTranslation($this->Lang)->body!=""):?>
            <li><?php echo CHtml::link( $item_n ,Yii::app()->controller->createUrl('site/'.$item->keyword, array('detail'=>$detail,'language'=>$this->Lang)));?>
            </li>
        <?php endif;?> 
    <?php } ?>
    <?php endif;?>
    ...
    <?php foreach($item->activeChildren as $child):?>
       <?php $item_name = $child->getTranslation($this->Lang)->name;?>
        <?php if (strpos($item->getTranslation($this->Lang)->getLink($this->Lang,$item->id), 'products') || 
                  strpos($item->getTranslation($this->Lang)->getLink($this->Lang,$item->id), 'portfolio') ||
