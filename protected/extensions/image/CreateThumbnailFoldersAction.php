...
class CreateThumbnailFoldersAction extends CAction{
    public function run(){
            ...
            foreach($modules as $module){
                echo $module->name.'<br>';
                $dir = Yii::app()->basePath.'/../images/'.$module->name;
                if (!file_exists($dir)){
                    mkdir($dir,0775);
                }
                ...
                @mkdir(Yii::app()->basePath.'/../site/images/',0777);
                @mkdir(Yii::app()->basePath.'/../site/uploads/',0777);
                @mkdir(Yii::app()->basePath.'/../site/images/'.$module->name,0775);
                @mkdir(Yii::app()->basePath.'/../site/uploads/'.$module->name,0775);
                foreach($module->thumbnails as $thumbnail){
                    @mkdir(Yii::app()->basePath.'/../site/images/'.$module->name.'/'.$thumbnail->name,0775);
