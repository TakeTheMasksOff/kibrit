...
class Banners extends Translatable
{
	...
	public function rules()
	{
		...
		return array(
			array('sort,type,user_date,color', 'required'),
			array('item_id, brands_id, sort, name', 'numerical', 'integerOnly'=>true),
			array('text, link, pic_name,thumbnail', 'length', 'max'=>255),
        ...
        public function types($key=NULL){
            $tmp= array(
                ...
                'front-right-banners'=>Yii::t('frontend.strings','Front right banner'),
                'left-banners'=>Yii::t('frontend.strings','Left banners'),
                'bottom-banner'=>Yii::t('frontend.strings','Bottom banner'),
            );
            if ($key)
