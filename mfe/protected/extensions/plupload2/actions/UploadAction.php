...
class UploadAction extends CAction {
    ...
    public function run() {
        ...
        $pic_name = $_POST['name']; //isset($_FILES['Gallery']) && isset($_FILES['Gallery']['name']) && isset($_FILES['Gallery']['name']["pic_name"]) ? $_FILES['Gallery']['name']["pic_name"] : $_POST['name'];
        $model = $picModel::model()->findByAttributes(array('pic_name'=>($pic_name)));

        $file = CUploadedFile::getInstance(new $picModel(), 'pic_name');
        //$file = new CUploadedFile($pic_name, $_FILES['file']['tmp_name'],$_FILES['file']['type'], $_FILES['file']['size'], $_FILES['file']['error']);
        if (!$model && $file) {

            if ($file->saveAs($this->basePath . $pic_name)) {
                $this->append($pic_name);
                $tmp = new $picModel();
